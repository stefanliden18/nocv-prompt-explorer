...
export default function ContactEdit() {
  ...
  useEffect(() => {
    if (sections) {
      ...
      const formIntro = sections.find(s => s.section_key === 'form_intro');
      
      if (hero) setHeroContent(hero.content_html);
      if (formIntro) setFormIntroContent(formIntro.content_html);
    }
  }, [sections]);
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const updates = [
        ...
        { section_key: 'form_intro', content_html: formIntroContent },
      ];
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">Redigera Kontakt-sidan</h1>
            <p className="text-muted-foreground mt-1">
              Använd editorn nedan för att uppdatera innehållet på Kontakt-sidan
            </p>
          </div>
          <Button 
        ...
        <Tabs defaultValue="hero" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="form_intro">Formulär-introduktion</TabsTrigger>
          </TabsList>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <TabsContent value="form_intro" className="mt-0">
                <Card>
                  <CardHeader>
                    <CardTitle>Formulär-introduktion</CardTitle>
                    <CardDescription>
                      Text som visas ovanför kontaktformuläret (valfritt)
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <RichTextEditor
                ...
                <CardContent>
                  ...
                  <TabsContent value="form_intro" className="mt-0">
                    <div 
                      className="prose prose-sm max-w-none dark:prose-invert"
                      dangerouslySetInnerHTML={{ 
                        __html: DOMPurify.sanitize(formIntroContent) 
                      }}
                    />
                  </TabsContent>
                </CardContent>
